    ...
    def _prepare_predictor(self):
        ...
        if self._engine == 'pytorch':
            self.inference_backend = PyTorchInferBackend(
                self._task_path, multilingual=False, device=self._device, use_fp16=self._use_fp16)#self._multilingual
...
if __name__ == '__main__':
    args = parse_args()
    args.schema = ['地块','城市', '区县','经度','纬度','面积','行业类型','风险评分','特征污染物','土壤类型','浓度','年份','深度']
    path = args.input
    uie = UIEPredictor(model=args.model, task_path=args.task_path, schema_lang=args.schema_lang, schema=args.schema, engine=args.engine, device=args.device,
                       position_prob=args.position_prob, max_seq_len=args.max_seq_len, batch_size=32, split_sentence=False, use_fp16=args.use_fp16)
    save = args.output
    for file in os.listdir(path):
        with open(path+file) as f, open(save+file,'w') as w:
            for line in f.readlines():
                line = re.sub(' ','',line)
                w.write(line+'\n')
                line = cut_sentences(line.strip())
                print(line)
                for item in uie(line):
                    if len(item) !=0:
                        w.write(json.dumps(str(item),ensure_ascii=False)+'\n')